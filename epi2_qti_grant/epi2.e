 ...
 *
 *	File Name:  @filename@   
 *	Developer:  P. Licato, F. Epstein
 *
 * $Source: epi.e $
 * $Revision: 1.183 $  $Date: 12/14/95 16:45:25 $
 */
...
int res_gd1; /* number of points in the diffusion waveform before refocusing pulse */
int res_gd2; /* number of points in the diffusion waveform after the refocusing pulse */
...
int wg_gxdr = 0 with {0, WF_MAX_PROCESSORS*2-1, , VIS, , };
int wg_gxdl = 0 with {0, WF_MAX_PROCESSORS*2-1, , VIS, , };
/* Y-diffusion pulses */
float a_gydl;
float a_gydr;
...
int wg_gydr = 0 with {0, WF_MAX_PROCESSORS*2-1, , VIS, , };
int wg_gydl = 0 with {0, WF_MAX_PROCESSORS*2-1, , VIS, , };
...
int wg_gzdr = 0 with {0, WF_MAX_PROCESSORS*2-1, , VIS, , };
int wg_gzdl = 0 with {0, WF_MAX_PROCESSORS*2-1, , VIS, , };
...
int fast_rec = 0 with { 0, 1, 0, VIS,
 "receiver selected: 0=standard recvr, 1=fast recvr",};
...
int waveform_type; /* specifies whether to play isotropic or linear encoding */
...
float ztarg = 1.0 with {0.0,10.0,1.0,VIS, "EPI read train logical z target",};
...
float focus_unwanted_delta_f = 440 with {0, 1000, 440, VIS, "Delta frequency of unwanted signal in focus [Hz]", };
...
@inline ARC.e ARCInit
    ...
    if ( strncasecmp("ezdwi",get_psd_name(),5) == 0 ) {
        if ( (cfsrmode == PSD_SR100) || (cfsrmode == PSD_SR120) || (cfsrmode == PSD_SR150) ) {
            epic_error( use_ermes, "EZDWI is not compatible with SR100, SR120 or SR150", EM_PSD_EZDWI_INCOMPATIBLE, EE_ARGS(0) ); 
            return FAILURE;
        } else {
            EZflag = PSD_ON;
        }
...
@inline MK_GradSpec.e GspecEval
    ...
    if(grad_spec_change_flag) {
        if(grad_spec_ctrl)config_update_mode = CONFIG_UPDATE_TYPE_SKIP;
            ...
            if (PSD_XRMW_COIL == cfgcoiltype) config_update_mode = CONFIG_UPDATE_TYPE_DVW_AMP321SR120;
	...
	pepolar = opuser13;
        ...
        if ((exist(opuser9) > 0.51) || (exist(opuser9) < 0.49))
        {
            ...
            /* Method 1: Shim volume Mode CFH + Global Shim */
            /* Method 2: opuser9 = 2 -- Shim volume Mode CFH + dual Local Shim */

            global_shim_method = (int)exist(opuser9);
        ...
        {
            /* CV9=0.5, Method 2: Slice Mode CFH + Global Shim */
        ...
        if ( (PSD_ON == type1_support) && (rfov_flag)  )
        {
            piuset |= use21;
            opuser21 = 1;
        ...
        {
            piuset &= ~use21;
            cvmod( opuser21, -MAXFLOAT, MAXFLOAT, 1.0, "User CV variable 21", 0, ""  );
...
@inline Muse.e MuseCVInitXYResUIOptions
    ...
    if ( (cfsrmode == PSD_SR50 || cfsrmode == PSD_SR77) && (vrgfsamp == PSD_ON) )
    {
        pifovnub = 6;
        pifovval2 = 240;
        pifovval3 = 280;
    ...
    if ( (cfsrmode == PSD_SR50 || cfsrmode == PSD_SR77) && (EZflag == PSD_ON) )
    {  
        pifovnub = 2;
        pifovval2 = 360;
        avminfov = 360;
...
@inline DTI.e DTI_read_grad_res /* granty edit to read waveform resolution */
...
@inline RTB0.e RTB0SupportMode
    ...
    /* HCSDM00155198 */
    if ((PSD_ON == tensor_flag) && (PSD_OFF == rfov_flag) && (PSD_OFF == opresearch) &&
...
@inline ARC.e ARCEval
    ...
    if(floatsAlmostEqualEpsilons(0, exist(opuser12), 2)) opuser12 = 60;
    read_grad_res(opuser12, &res_gd1, &res_gd2);
    pw_gd1 = res_gd1*4;
    pw_gd2 = res_gd2*4;
    ...
    if( (cfgradamp == 8919) && (epi_flair == PSD_OFF) &&
    ...
    if( (cfgradamp == 8919) && (exist(opdiffuse) == PSD_ON) &&
        ((value_system_flag == VALUE_SYSTEM_SVEM) ||
         (value_system_flag == VALUE_SYSTEM_SVDM)) && 
    ...
    if (nshots_locks == PSD_ON) {
        if ( (cfsrmode==PSD_SR17) || (cfsrmode==PSD_SR20) || (cfsrmode==PSD_SR25) )
            ...
            if (exist(opxres)<=128)
                ...
                min_nshots = 1;
        else if ((cfsrmode==PSD_SR100) || (cfsrmode==PSD_SR120))
            if (exist(opxres)==512)
                min_nshots = 1;
            else
                min_nshots = 1;
...
cveval1( void )
{
    ...
    {
        temp_slthick = exist(opslthick) + 1.0 * exist(opslspace);
    }
    else
    {
...
@inline AutoAdjustTR.e AutoAdjustTRDebug
    ...
    if ( (cfsrmode == PSD_SR20 || cfsrmode == PSD_SR25 ) || ( (cfsrmode == PSD_SR50 || cfsrmode == PSD_SR77) && (EZflag == PSD_ON)) )
    {
        avminfov = 360;
        avmaxfov = 360;
    }
    else if ( (cfsrmode == PSD_SR50 || cfsrmode == PSD_SR77) && (vrgfsamp == PSD_ON)) 
    {
        avminfov = 240;
        if(cfsrmode == PSD_SR77)
        {
...
@inline AutoAdjustTR.e PulsegenonhostSwitch
    ...
    if (rfov_flag)
    {
        if ((rfov_maxnslices >= DEFAULT_MAXNSLICES_RFOV) && (exist(optr) >= FOCUS_ONE_ACQ_MIN_TR) && (!value_system_flag) && (!k15_system_flag))   /* HCSDM00397660 */
        {
...
@inline Inversion_new.e InversionEval1
    ...
    if (t1flair_flag == PSD_OFF)
    {
        ...
        /* HCSDM00398231 */
        if ( (existcv(optracq) && (exist(optracq) > 0)) && rfov_flag && (exist(optr) < FOCUS_ONE_ACQ_MIN_TR) && (sl_acq > DEFAULT_MAXNSLICES_RFOV) && (!value_system_flag) && (!k15_system_flag) )
        {
...
STATUS setUserCVs()
{
    if (PSD_ON == exist(opdiffuse))
    {
        ...
        if((mux_flag || ssgr_flag) &&
           (!irprep_flag && !exist(opspecir) && !rfov_flag && !ss_rf1_compatible))
        {
            cvoverride(opfat, PSD_ON, PSD_FIX_ON, PSD_EXIST_ON);
        }
        else
        ...
        else 
        {
            ...
            case PSD_60_CM_COIL:
                ...
                case PSD_SR120:
                    ...
                    ***/
                    if(((VALUE_SYSTEM_SVEM == value_system_flag) || (VALUE_SYSTEM_SVEM == value_system_flag)) && (exist(opslthick) >= 20))
                    {
                        allowed_max_bval = MAXB_10000;
                    }
                    /*********************************/                    
...
@inline MK_GradSpec.e GspecCheck
    ...
    if ( ((rhnframes + rhhnover) % exist(opnshots)) != 0 ) { 
        ...
        epic_error( use_ermes, "Combination of opnshots and opyres incompatible try opyres of %d\n.", EM_PSD_EPI_NSHOTS_YRES_INCOMPATIBLE, EE_ARGS(1), INT_ARG, newyres );
        avminyres = newyres;
        avmaxyres = newyres;
        return ADVISORY_FAILURE;
    }
...
/* TOMOD: need to add errmsgs */
    ...
    if ( (!isCategoryMatchForAnatomy(opanatomy, ATTRIBUTE_CATEGORY_HEAD) && opanatomy) 
         ...
         && (PSD_ON == mux_flag) && (exist(opaccel_mb_stride)>1) && (PSD_ON != exist(opresearch)) )
    {
        epic_error(use_ermes, "%s is incompatible with %s", EM_PSD_INCOMPATIBLE,
...
@inline ARC.e ARCrh
    ...
    *********************/
    ...
    ia_gxdl = a_gxdl*MAX_PG_IAMP;
    ia_gxdr = a_gxdr*MAX_PG_IAMP;
    ia_gydl = a_gydl*MAX_PG_IAMP;
    ia_gydr = a_gydr*MAX_PG_IAMP;
    ia_gzdl = a_gzdl*MAX_PG_IAMP;
    ia_gzdr = a_gzdr*MAX_PG_IAMP;
...
@inline loadrheader.e rheaderpredownload
    ...
    if( enable_acoustic_model && (PSD_OFF == oploadprotocol) )
    {
        ...
        if( FAILURE == minseq(&min_acoustic, gradx, GX_FREE, grady, GY_FREE, gradz, GZ_FREE, &loggrd, seqEntryIndex,
                    tsamp,avail_image_time, use_ermes, seg_debug) )
        {
...
@inline Prescan.e PSfilter
    ...
    if (vrgfsamp == PSD_ON) {
        dacq_adjust = (float)pw_gxw/2.0 + (float)(pw_gxwad) -
            (float)(pw_gyba + pw_gyb + pw_gybd)/2.0 - tsp*((float)rhfrsize - 1.0)/2.0;
    } else {
    ...
    {    
        ...
        if( weighted_avg_grad == PSD_ON && pgen_calc_bval_flag == PSD_OFF && AVERAGE_POWER == encode_mode ) 
        {
		    ...
		    if(floatsAlmostEqualEpsilons(WAVE_HOST[kk], 1, 2)){
                    	scale_difx += TENSOR_HOST[0][kk] * TENSOR_HOST[0][kk] * max_bval/sde_max_bval;
                    	scale_dify += TENSOR_HOST[1][kk] * TENSOR_HOST[1][kk] * max_bval/sde_max_bval;
                    	scale_difz += TENSOR_HOST[2][kk] * TENSOR_HOST[2][kk] * max_bval/sde_max_bval;
		    } else if (floatsAlmostEqualEpsilons(WAVE_HOST[kk], 3, 2)) {
                   	scale_difx += TENSOR_HOST[0][kk] * TENSOR_HOST[0][kk] * max_bval/ide_max_bval;
                    	scale_dify += TENSOR_HOST[1][kk] * TENSOR_HOST[1][kk] * max_bval/ide_max_bval;
                    	scale_difz += TENSOR_HOST[2][kk] * TENSOR_HOST[2][kk] * max_bval/ide_max_bval;
		    }
 		    /* end edit */
                    if ( weighted_avg_debug == PSD_ON ) 
                    {
