...
/*
 *  GE Medical Systems
 *  Copyright (C) 2001 The General Electric Company
 *  
...
@host DTI_host_funcs
static INT read_grad_res(float wave_number, int* res_gd1, int* res_gd2);
...
float WAVE_HOST[MAX_DIRECTIONS + MAX_T2];
...
#else
	    sprintf(fname,"/usr/g/research/grant/cdiff/WAVES/wave%d.dat",(int)wave_number);
#endif 
...
#endif /* __STDC__ */ 
{
    ...
    optimizedTEFlag = (isRioSystem()) ? PSD_ON : exist(opmintedif);
...
get_diffusion_time( void )
{
    ...
    if ((opdiffuse == PSD_ON)|| (tensor_flag == PSD_ON)) {
        ...
        if(mpg_opt_flag)
        {
            ...
            minRamp = minRamp * mpg_opt_derate;
        }
        else
        {
            mpg_opt_derate = 1.0;
        ...
        /*** SVBranch: HCSDM00259119 -  mpg opt ***/
        if(mpg_opt_flag) minRamp = minRamp * mpg_opt_derate;
        /**********************/
...
static STATUS update_sse_diffusion_time( void )
{
    ...
    {
        pw_wgxdl = get_sse_waittime() + RDN_GRD((exist(opte) - cur_minte)/2);
        pw_wgydl = pw_wgxdl;
        pw_wgzdl = pw_wgxdl;
    }
...
set_tensor_orientations( void )
{
    ...
    if( (read_from_file == PSD_ON)  && (tensor_flag == PSD_ON )) {
        ...
        int num_tensor_len;
#ifndef SIM
        const char *tensor_datapath="/usr/g/research/nqti";/* path to tensor.dat files - hw  */
        ...
        for(i= num_B0; i < num_tensor + num_B0; i++)
        {
            ...
            if (mag[i]> MAX_TENSOR_VECTOR_MAG*sqrt(3)) /* granty edit to allow ide to run at full amplitude */
            {
